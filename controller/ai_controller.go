...
// If userGroqKey is non-empty, it takes precedence over the server-side GROQ_API_KEY env var.
func transcribeAudio(audioPath string, userGroqKey ...string) (string, error) {
	apiKey := ""
	if len(userGroqKey) > 0 && userGroqKey[0] != "" {
		apiKey = userGroqKey[0]
	} else {
