...
export function ShaderBackground({ className }: ShaderBackgroundProps) {
  if (Platform.OS !== "web") {
    return <View className="absolute inset-0 bg-tier-bg -z-10" />;
  }

  return <ShaderBackgroundWeb className={className} />;
}

function ShaderBackgroundWeb({ className }: { className?: string }) {
  ...
  // Fragment shader source code with TierLog indigo/violet color palette
  const fsSource = `
    ...
    const vec4 gridColor = vec4(0.15, 0.18, 0.3, 0.3);
    const float scale = 4.5;
    const vec4 lineColor = vec4(0.45, 0.35, 0.95, 0.9); // TierLog Indigo-Violet Glow
    ...
    void main() {
      ...
      vec4 lines = vec4(0.0);
      // Immersive Dark Theme Gradient Palette (#020617 to #0f0a29)
      vec4 bgColor1 = vec4(0.01, 0.02, 0.08, 1.0);
      vec4 bgColor2 = vec4(0.05, 0.03, 0.15, 1.0);
      ...
      for(int l = 0; l < linesPerGroup; l++) {
        ...
        float circle = drawCircle(circlePosition, 0.012, space) * 3.5;
  ...
  return (
    <div className={cn("fixed inset-0 w-full h-full pointer-events-none overflow-hidden -z-10", className)}>
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full opacity-70" />
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-tier-bg/40 to-tier-bg opacity-90" />
    </div>
  );
}
