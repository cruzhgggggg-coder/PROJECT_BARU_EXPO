...
function MotionNativeDiv({ children, style, initial, animate, transition, variants, custom, exit, ...rest }: MotionProps) {
  ...
  const motiTransition = transition ? {
    ...
    duration: transition.duration ? (transition.duration < 10 ? transition.duration * 1000 : transition.duration) : 300,
    delay: (transition.delay ? (transition.delay < 10 ? transition.delay * 1000 : transition.delay) : 0) + ((custom ?? 0) * 150),
  } : undefined;
