...
import { AlertCircle, Archive, BookOpen, CheckCircle, ChevronLeft, ChevronRight, Clock, MessageSquare, User } from "lucide-react-native";
      ...
      >
        ...
        {isMobile ? (
          ...
          <View className="w-full flex-col gap-4">
            ...
            <View className="flex-row gap-5 items-start">
              {/* Column 1 (Left): Student Roster List */}
              <GlassCard className="w-[300px] shrink-0 p-5 max-h-[850px] flex-col">
                <SectionHeader
                  icon={<User color="#6366F1" size={18} />}
                  title="Student Roster"
                />
                <ScrollView className="flex-1" showsVerticalScrollIndicator={true} contentContainerStyle={{ paddingBottom: 16 }}>
                  {renderStudentRoster(false)}
                </ScrollView>
              </GlassCard>

              {/* Column 2 (Right): Details Pane */}
              <View className="flex-1 min-w-0 items-start">
                {selectedStudent ? (
                  <View className="w-full gap-4">
                    {/* Selected Student Banner Info */}
                    <View className="bg-tier-surface-sunken border border-tier-divider-light rounded-tier-xl p-4 gap-3">
                      <View className="flex-row items-center gap-3">
                        <View className="w-[42px] h-[42px] rounded-tier-full bg-tier-accent-primary/10 border border-tier-accent-primary/20 items-center justify-center">
                          <User color="#6366F1" size={20} />
                        </View>
                        <View className="flex-1">
                          <Text className="text-tier-text-primary text-base font-extrabold tracking-tight">{selectedStudent.name}</Text>
                          <Text className="text-tier-text-secondary text-[11px] font-semibold">
                            NIM: {selectedStudent.nim} · {selectedStudent.prodi}
                          </Text>
                        </View>
                        {(() => {
                      ...
                      {selectedStudent.thesis_title && (
                        <View className="border-t border-tier-divider-light pt-2.5 mt-1">
                          <Text className="text-tier-text-secondary text-[11px] leading-4.5 font-medium">
                            {selectedStudent.thesis_title}
                          </Text>
                        </View>
                      )}
                    ...
                    {/* Guidance History Card */}
                    <GlassCard className="w-full p-5 max-h-[750px] flex-col">
                      <View className="flex-row items-center gap-2 pb-3 border-b border-tier-divider-light mb-3">
                        <BookOpen color="#6366F1" size={16} />
                        <Text className="text-tier-text-primary text-sm font-extrabold tracking-tight">Guidance History</Text>
                      </View>
                      <ScrollView className="flex-1" showsVerticalScrollIndicator={true} contentContainerStyle={{ gap: 16, paddingBottom: 16 }}>
                        {renderStudentDetailOverview()}
                  ...
                  <GlassCard className="flex-1 items-center justify-center p-[60px] gap-4 min-h-[500px] w-full">
                    <User color="#64748B" size={40} />
                    <Text className="text-tier-text-secondary text-sm text-center font-semibold max-w-[280px] leading-[22px]">
                      Select a student from the roster to view detailed guidance history and session logs.
                    </Text>
                  </GlassCard>
                )}
              </View>
